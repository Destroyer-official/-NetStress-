...
class FPGAAccelerator(HardwareAcceleratorBase):
    ...
    def _load_bitstream(self) -> bool:
        ...
        try:
            logger.info(f"Loading FPGA bitstream for {self.fpga_vendor} device")
            
            # Configure FPGA capabilities based on vendor and detected devices
            bitstream_config = self._configure_fpga_bitstream()
            
            # Initialize hardware engines
            self._initialize_fpga_engines(bitstream_config)
            
            self.bitstream_loaded = True
            self.fpga_capabilities = bitstream_config
            
            logger.info(f"FPGA bitstream loaded with capabilities: {list(bitstream_config.keys())}")
            return True
        ...
        try:
            ...
            if self._detect_rdma_devices():
                if self._setup_rdma_context():
                    self.acceleration_enabled = True
                    logger.info("RDMA acceleration initialized")
                    return True
                    
            logger.warning("No RDMA acceleration available")
            return False
