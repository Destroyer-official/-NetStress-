...
void test_transport_checksum(void) {
    ...
    uint16_t checksum = calculate_transport_checksum(src_ip, dst_ip, protocol, udp_data, 8);
    ...
    uint16_t tcp_checksum = calculate_transport_checksum(src_ip, dst_ip, protocol, tcp_data, 20);
...
void test_raw_socket_creation(void) {
    ...
    if (sock >= 0) {
        ...
        TEST_ASSERT(result == 0 || result == -1, "IP_HDRINCL setting returned valid result");
        
        raw_socket_close(sock);
        TEST_ASSERT(1, "Raw socket closed successfully");
    } else {
        TEST_ASSERT(1, "Raw socket creation failed as expected (likely no privileges)");
    }
...
void test_utility_functions(void) {
    ...
    TEST_ASSERT(pin_result == 0 || pin_result == -1, "CPU pinning returned valid result");
}
...
void test_backend_detection(void) {
    ...
    TEST_ASSERT_STR_EQ(backend_name(BACKEND_RAW_SOCKET), "raw_socket", "Raw socket name");
    TEST_ASSERT_STR_EQ(backend_name(BACKEND_SENDMMSG), "sendmmsg", "sendmmsg name");
    TEST_ASSERT_STR_EQ(backend_name(BACKEND_IO_URING), "io_uring", "io_uring name");
    ...
    TEST_ASSERT_STR_EQ(backend_name((backend_type_t)999), "unknown", "Unknown backend name");
}
...
void test_sendmmsg_batch(void) {
    ...
    TEST_ASSERT(sent_same >= 0, "sendmmsg_batch_same_dest should return non-negative result");
    TEST_ASSERT(sent_same <= 3, "sendmmsg_batch_same_dest should not send more than requested");
...
void test_driver_stats(void) {
    ...
    TEST_ASSERT(sizeof(driver_stats_t) >= 20, "driver_stats_t should be at least 20 bytes");
    TEST_ASSERT(sizeof(driver_stats_t) % 4 == 0, "driver_stats_t should be 4-byte aligned");
}
...
void test_stub_functions(void) {
    /* These should all return -1 or appropriate error values when features are disabled */
    ...
    TEST_ASSERT_EQ(dpdk_send_burst(0, NULL, NULL, 0), -1, "DPDK send stub should return -1");
    TEST_ASSERT_EQ(dpdk_recv_burst(0, NULL, 0), -1, "DPDK recv stub should return -1");
    TEST_ASSERT_EQ(cleanup_dpdk(), 0, "DPDK cleanup stub should return 0");
    
    driver_stats_t stats;
    TEST_ASSERT_EQ(dpdk_get_stats(0, &stats), -1, "DPDK stats stub should return -1");
#endif
    ...
    TEST_ASSERT_EQ(af_xdp_send_batch(NULL, NULL, 0), -1, "AF_XDP batch send stub should return -1");
    ...
    TEST_ASSERT_EQ(io_uring_send_single(NULL, 0, NULL), -1, "io_uring send stub should return -1");
    TEST_ASSERT_EQ(io_uring_send_batch(NULL, NULL, NULL, 0), -1, "io_uring batch send stub should return -1");
    TEST_ASSERT_EQ(cleanup_io_uring(), 0, "io_uring cleanup stub should return 0");
    
    driver_stats_t stats;
    TEST_ASSERT_EQ(io_uring_get_stats(&stats), -1, "io_uring stats stub should return -1");
#endif
}
...
void test_packet_validation(void) {
    ...
    TEST_ASSERT(sizeof(test_packet_udp) >= 28, "UDP test packet should be at least 28 bytes");
    ...
    TEST_ASSERT(sizeof(test_packet_tcp) >= 40, "TCP test packet should be at least 40 bytes");
    ...
    TEST_ASSERT_EQ(src_ip, htonl_test(0xC0A80101), "Source IP should be 192.168.1.1");
    TEST_ASSERT_EQ(dst_ip, htonl_test(0xC0A80102), "Dest IP should be 192.168.1.2");
}
...
void test_checksum_performance(void) {
    ...
    printf("  Checksum performance: %d iterations in %.3f seconds (%.1f ops/sec)\n",
